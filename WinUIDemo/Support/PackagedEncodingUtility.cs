...
public static class PackagedEncodingUtility
{
    static Encoding[] _allSupportedANSIEncodings;
    static Encoding _systemDefaultANSIEncoding;
    static Encoding _currentCultureANSIEncoding;

    // https://docs.microsoft.com/en-us/windows/win32/intl/code-page-identifiers
    // Name is using format of "<CountryOrRegion> (<.NET Name>)"
    static readonly Dictionary<int, string> ANSIEncodings = new Dictionary<int, string>() {
    ...
    public static string GetEncodingNameFallback(Encoding encoding)
    {
        ...
        else
        {
            try
            {
                ...
                Debug.WriteLine($"FoundUnlistedEncoding '{encoding.WebName}' ({encoding.CodePage})");
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"FailedToGetNameOfUnlistedEncoding: {ex.Message}");
            }
        }
    ...
    public static Encoding GetEncodingByNameFallback(string name)
    {
        ...
        {
            Debug.WriteLine($"FailedToGetEncoding: {ex.Message}");
        }
    ...
    public static bool TryGetSystemDefaultANSIEncoding(out Encoding encoding)
    {
        ...
        {
            Debug.WriteLine($"FailedToGetSystemDefaultANSIEncoding: {ex.Message}");
        }
    ...
    public static bool TryGetCurrentCultureANSIEncoding(out Encoding encoding)
    {
        ...
        {
            Debug.WriteLine($"FailedToGetCurrentCultureANSIEncoding: {ex.Message}");
        }
    ...
    public static Encoding[] GetAllSupportedANSIEncodings()
    {
        ...
        foreach (var (codePage, encodingName) in ANSIEncodings)
        {
            ...
            {
                Debug.WriteLine($"FailedToGetANSIEncoding: {ex.Message}");
            }
        }
